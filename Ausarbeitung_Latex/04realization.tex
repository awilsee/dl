...
To retrain a model the python script \textit{code/scripts/retrain.py} from the github repository of this project is used and executed as shown in \listref{retrain}. This script is taken from \citep{TensorflowDemo} and is extended with some time measuring markers. All relevant parameters are explained later in this section. However, for retraining the following command was used \textit{python -m scripts.retrain -h}. 
...
The graph of the model can also be visualized in TensorBoard. The retrained graph of the MobileNet model including the training nodes is depicted in \figref{graphMobilenet050-700}. In the appendix \subsecref{MobileNet retrained graph - detailed}, there is a more detailed overview with all convolutional layer. In \subsecref{MobileNet retrained graph - depthwise}, the depthwise convolution containing its depthwise and pointwise convolutional layers can be found as described in \subsecref{Common Models in Deep Learning Applications}. \\
The whole retrained graph of the InceptionV3 model is depicted in the appendix under \subsecref{InceptionV3 retrained graph}. Furthermore, under \subsecref{InceptionV3 retrained graph - mixed_layer} a detailed view on a mixed_layer and under \subsecref{InceptionV3 retrained graph - conv layer} it's specific operations within a convolutional layer are shown. \\

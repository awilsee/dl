...
To use TensorFlow with GPU support, the older CUDA Toolkit in version 8.0 and cuDNN in version 6.0 must be installed. \\
...
The installation of CUDA is shown in \listref{cuda}.
...
If a newer version of CUDA is already installed, it can be removed by the prompts depicted in \listref{cudaUninstall}
...
To verify the installation, the verification environment must be set up by following commands (refer to \listref{cudaVerifyEnv}). 
...
Thereafter, \textit{deviceQuery} was executed which is placed in the directory\textit{NVIDIA_CUDA-8.0_Samples/bin/}. The output is shown in \figref{GPUdeviceQuery}. The important outcomes are that a device was found and that the final test passed. \\
...
In order to check if the system and the CUDA device are able to communicate with each other correctly, \textit{bandwidthTest} was executed. The output is shown in \figref{GPUBandwidth}. \\
...
To verify that cuDNN is working correctly, mnistCUDNN was compiled and executed  with the following commands (\listref{cuDNNverify}).
...
As a last step, the libcupti-dev library was installed and exported by the following commands \listref{libcupti}.
...
In this section the whole build and re-/training process is described. Additionally, several comparisons of the execution time on different hardware setups are done. \\

After setting up the entire software environment independent of a Python/ Bazel/ GPU/ CPU based setup, the models can be retrained. For several reasons as already explained in \subsecref{Key requirements for an appropriate dataset} and \subsecref{Dataset} pretrained models are used for this project. The script used for retraining will automatically download the desired pretrained model. \\
...
To retrain a model the python script \textit{code/scripts/retrain.py} from the github repository of this project is used and executed as shown in \listref{retrain}. This script is taken from the 'tensorflow-for-poets'-tutorial and is extended with some time measuring markers. All relevant parameters are explained later in this section. However, for retraining the following command was used \textit{python -m scripts.retrain -h}. 
...
Listing \ref{list:bottleneck_gpu} shows the workload of the GPU during calculation which can be produced by executing \textit{nvidia-smi -l 1}. It works at 38\% capacity and uses about 3160 MiB memory. Whereas the CPU is only utilized at 110\% of the maximum 800\% because of four cores with hyper threading which can be seen in the upper part of \figref{bottlenecksCPUUsage}. The lower part of this figure shows about 470\% workload if calculating everything with the desktop-CPU only. \\
...
If using Python and Bazel, the same version of TensorFlow has to be chosen, otherwise compatibility issues can occur. \\

During the retraining process the accuracy, cross entropy and some more data are written to the summaries directory and can be visualized with TensorBoard which is automatically installed with TensorFlow. In order to start the board, following command is executed \listref{tensorboard}.
...
The graph of the model can also be visualized in TensorBoard. The retrained graph of the MobileNet model including the training nodes is depicted in \figref{graphMobilenet050-700}. In the appendix \subsecref{}, there is a more detailed overview with all convolutional layer. In \subsecref{}, the depthwise convolution containing its depthwise and pointwise convolutional layers can be found as described in \subsecref{Common Models in Deep Learning Applications}. \\
The whole retrained graph of the MobileNet model is depicted in the appendix \subsecref{}. Furthermore, in \subsecref{} a detailed view on a mixed_layer and in \subsecref{} it's specific operations within a convolutional layer are shown. \\
...
In \figref{inception5000LR} the differences of retraining the InceptionV3 model with 5000 training steps and different learning rates is shown. For MobileNets it behaves similar. The final results experimenting with different training steps and learning rates are described in \secref{Evaluation}. \\
...
The first option, which will be called in following as \textit{opt1}, is only for runing the \textit{optimize_for_inference}-script of the official tensorflow github repository. The corresponding call is shown in \listref{optimize_graph}.
...
The next attempt is to use the already optimized graph of \textit{opt1} and additionally quantizes it by rounding the weights. The \textit{quantize_graph.py}-script of tensorflow-for-poets-2-tutorial can be called as shown in \listref{quantize_graph}. This will make the model compressible, which will reduce the model size significantly. Thus every mobile app compresses the package before distribution.
...
To compare the improvement, use the gzip command in \listref{gzip} on the model of \textit{opt1} and also on the new one. 
...
The detailed result of both graphs is shown in \listref{gzip_res}. So, the \textit{opt2} model is 65,4\% lower in size. This is also independent of the model. In the example of an InceptionV3 model the size is dropped from about 88 MB to only 24 MB which is a great improvement. 
...
The first two ways of optimization are described in the tensorflow-for-poets-2-tutorial. \\
...
Calling the commands in \listref{transform_graph_opt3} strips all unused nodes and folds batch normalization ops, so very similar to \textit{opt1}. In the following content this will be \textit{opt3}.

...
To retrain a model the python script \textit{code/scripts/retrain.py} in the github repository of this project is used and executed as shown in \listref{retrain}. This script is taken from the 'tensorflow-for-poets'-tutorial and is extended with some time measuring stuff. All relevant parameters are explained later in this section. However, for all possible options just execute \textit{python -m scripts.retrain -h}. 
...
The whole retrained graph of the MobileNet model is depicted in the appendix under \subsecref{}. Furthermore, under \subsecref{} a detailed view on a mixed_layer and under \subsecref{} it's specific operations within a convolutional layer is shown. \\

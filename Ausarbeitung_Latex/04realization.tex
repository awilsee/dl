...
The kernel headers and development packages for the currently running kernel has to be installed using following command in \listref{linuxheaders}.
...
In order to check if the system and the CUDA device are able to communicate correctly, execute \textit{bandwidthTest}. The Output should look like in \figref{GPUBandwidth}.
...
For downloading cuDNN on \url{https://developer.nvidia.com/cudnn}, a registration for the NVIDIA Developer Program is needed. Download the three deb-files of cuDNN 6.0 for CUDA 8.0 for your corresponding platform environment. Afterwards install it with the prompts of \listref{cuDNN}.
...
In \figref{inception5000LR} the differences of retraining the InceptionV3 model with 5000 training steps and different learning rates is shown. For MobileNets it looks basically similar. The final results of trying out different training steps and learning rates are described in \secref{Evaluation}.
